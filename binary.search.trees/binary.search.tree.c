...
void delete(struct node** tree, uint32_t x) {
    ...
    if ((*tree)->item == x) {
        ...
        if ((*tree)->left == NULL && (*tree)->right == NULL) {
            printf("node has no children, deleting...\n"); 

            // has no children, procede checking where the node is (right or left of parent)
            if ((*tree)->parent->left != NULL) {
                // since we have already found the node, checking if the child item matches x is redundant
                printf("node was on the left of %p\n", (*tree)->parent);
                (*tree)->parent->left = NULL;
            } else {
                printf("node was on the right of %p\n", (*tree)->parent);
